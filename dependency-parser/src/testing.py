...
def LAS_score(pred_tensor, gold_tensor, pred_labels, gold_labels):
    ...
    for i in range(length):
        if pred_tensor[i] == gold_tensor[i]:
            if pred_labels[i].data == gold_labels[i]:
                count += 1
    return count, length
...
if __name__ == '__main__':
    ...
    network_model = torch.load('../data/vndt/weights/trained_from_2024-06-09_14-48-41/latest_weights')
    l2i = json.load(open('../data/' + dataset + '/labels.json', 'r'))

    print('Weights loaded, Begin predicting on the test set')
    score = test(network_model, l2i, '../data/' + dataset + '/test.json')
    print('SCORE: ', score)
